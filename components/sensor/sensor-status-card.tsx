...
export function SensorStatusCard({ selected, deviceLabel, onPressAction }: Props) {
  const scheme = useColorScheme() ?? 'light';
  const palette = COLORS[scheme];

  const iconName = selected ? 'checkmark.circle.fill' : 'exclamationmark.triangle.fill';
  ...
  return (
    <View style={[styles.card, { backgroundColor: palette.surface, borderColor: palette.surfaceBorder }]}>
      <View style={[styles.iconBadge, { backgroundColor: badgeBg }]}>
        <IconSymbol name={iconName} size={26} color={badgeColor} />
      </View>
      <View style={styles.textBlock}>
        <ThemedText style={[styles.label, { color: palette.label }]}>MOTION SENSOR</ThemedText>
      ...
      {selected ? (
        <Pressable
          ...
          <ThemedText style={[styles.action, { color: palette.accent }]}>{actionText}</ThemedText>
        </Pressable>
      ) : null}
    </View>
  );
